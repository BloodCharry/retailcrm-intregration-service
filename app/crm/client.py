...
class RetailCRMClient:
    def __init__(self, base_url: str, api_key: str | None, site: str | None, timeout: float = 10.0):
        if not api_key:
    ...
    async def _request(
        ...
        try:
            ...
            logger.error("retailcrm_http_error", error=str(e), url=url, request_id=request_id)
            raise
    ...
    async def list_customers(self, filters: dict, page: int = 1, limit: int = 20) -> dict:
    ...
    async def list_orders(self, filters: dict, page: int = 1, limit: int = 20) -> dict:
        params = {"page": page, "limit": limit}
        if filters:
            params.update({f"filter[{k}]": v for k, v in filters.items()})
