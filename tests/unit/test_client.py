...
async def test_client_requires_api_key():
    with pytest.raises(ValueError):
        RetailCRMClient(base_url="https://example.com/api/v5", api_key=None, site="test")


@pytest.mark.asyncio
async def test_list_customers_adds_api_key(monkeypatch):
    client = RetailCRMClient("https://example.com/api/v5", "testkey", site="test")

    async def fake_request(method, url, params=None, data=None, headers=None):
        return httpx.Response(200, json={"ok": True, "params": params, "headers": headers})

    monkeypatch.setattr(client._client, "request", fake_request)

    result = await client.list_customers(filters={"email": "test@example.com"})
    assert result["ok"] is True
    assert result["headers"]["X-API-KEY"] == "testkey"
